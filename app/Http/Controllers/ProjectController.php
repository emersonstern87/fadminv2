...
class ProjectController extends Controller
{
  ...
  { 
    ...
    try {
        DB::beginTransaction();
          ...
          //Insert Activity
          (new Activity)->store('Project', $insertId, 'user', Auth::user()->id, __('A new project has been created'));
  ...
  public function details($id)
  {
      ...
      $data['project'] = DB::table('projects')
        ->where('projects.id',$id)
        ->select('projects.*','dm.first_name','dm.last_name','ps.name as status_name')
        ->leftJoin('customers as dm','dm.id','=','projects.customer_id')
        ->leftJoin('project_statuses as ps','ps.id','=','projects.project_status_id')
        ->first();
      if (empty($data['project'])) {
  ...
  public function mailToCustomer($projectDetails,$customer_id)
  {
    ...
    if (isset($customerData->email) && !empty($customerData->email) && filter_var($customerData->email, FILTER_VALIDATE_EMAIL)) {
      ...
      return $this->email->sendEmail($customerData->email, $subject, $message);
    }
    ...
    foreach ($members as $value) {
      if (Auth::user()->id != $value->user_id) {
        if (!in_array($value->user_id, $oldMembers)) {
          ...
          $response = $this->email->sendEmail($assigne->email, $subject, $message);
  ...
  {
    $data = ['menu' => 'project', 'header' => 'project', 'navbar' => 'task'];
    $data['project'] = DB::table('projects')
                        ->leftJoin('project_statuses as ps', 'ps.id', '=', 'projects.project_status_id')
