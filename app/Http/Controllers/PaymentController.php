...
use App\Model\SupplierTransaction;
...
class PaymentController extends Controller
{
    ...
    **/
    ...
    public function sendPaymentInformationByEmail(Request $request)
    {
        ...
        if ($emailConfig->status == 0 && $emailConfig->email_protocol == 'smtp') {
        ...
        if (isset($request['payment_pdf']) && $request['payment_pdf'] == 'on') {
            ...
            $emailResponse = $this->email->sendEmail($request['email'], $request['subject'], $request['message']);
            if ($emailResponse['status'] == false) {
                return redirect()->back()->withInput()->withErrors(['fail' => $emailResponse['message']]);
             }
        }
    ...
    public function emailPurchPaymentInformation(Request $request)
    {
        ...
        if (isset($request['payment_pdf']) && $request['payment_pdf'] == 'on') {
            ...
            $emailResponse = $this->email->sendEmailWithAttachment($request['email'], $request['subject'], $request['message'], $invoiceName);
