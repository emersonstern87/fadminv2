...
use App\DataTables\CustomerInvoiceListDataTable;
use App\DataTables\CustomerListDataTable;
use App\DataTables\CustomerOrderListDataTable;
use App\DataTables\CustomerPaymentListDataTable;
use App\DataTables\CustomerProjectListDataTable;
use App\DataTables\CustomerTaskListDataTable;
use App\DataTables\CustomerTicketListDataTable;
...
use App\Exports\customerTicketExport;
use App\Exports\customerPaymentExport;
use App\Exports\customerQuotationExport;
use App\Exports\customerLedgerExport;
use App\Model\Country;
use App\Model\Currency;
use App\Model\CustomerBranch;
...
class CustomerController extends Controller
{
    ...
    {
        if (!isset($request->type) && $request->type != md5('pos') && $request->email != null) {
            $this->validate($request, [
                'first_name'      => 'required|max:30',
                'last_name'       => 'required|max:30',
                'email'           => ['unique:customers,email', new CheckValidEmail],
        ...
        try {
            ...
            if (isset($request->sendMail) && isset($request->email) && !empty($request->email) && validateEmail($request->email)) {
                ...
                $emailResponse = $this->email->sendEmail($request->email, $subject, $message);
    ...
    public function invoice($id)
    {
        ...
        foreach ($invoiceSummery['amounts'] as $key => $value) {
            ...
            $overDueTotalAmount = !empty($data['overDue'][$key]['totalAmount']) ? $data['overDue'][$key]['totalAmount'] : 0;
            $overDueTotalPaid = !empty($data['overDue'][$key]['totalPaid']) ? $data['overDue'][$key]['totalPaid'] : 0;
    ...
    public function updatePassword(Request $request)
    {
        ...
        if (isset($request->sendmail) && filter_var($customerPasswordToUpdate->email, FILTER_VALIDATE_EMAIL)) {
            ...
            // Send Mail to the customer
            $emailResponse = $this->email->sendEmail($customerPasswordToUpdate->email, $subject, $message);
    ...
    {
        $data['address'] = CustomerBranch::with('billingCountry', 'shippingCountry')
    ...
    public function ticket($id)
    {
        ...
        if ((isset($from) && !empty($from)) || (isset($to) && !empty($to)) || (isset($allproject) && !empty($allproject)) || (isset($alldepartment) && !empty($alldepartment))) {
            $data['exceptClickedStatus'] = $exceptClickedStatus = (new Ticket)->getExceptClickedStatus($allstatus, $id);
