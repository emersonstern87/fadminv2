...
use App\Model\IncomeExpenseCategory;
...
class BankController extends Controller
{
    ...
    public function editAccount($tab, $account_id)
    {
        ...
        $data['startDate'] = $startDate = isset($_GET['from']) ? $_GET['from'] : date("Y-m-d", strtotime ("-30 day", strtotime(date('d-m-Y') )));

        $data['endDate'] = $endDate = isset($_GET['to']) ? $_GET['to'] : date('Y-m-d');
        $data['type'] = Transaction::distinct()->get(['transaction_method']);
        $data['modeVal']  = $mode = isset($_GET['mode']) ?$_GET['mode']: null;
        $data['typeVal']  = $type = isset($_GET['type']) ?$_GET['type']: null;
        ...
        $amount = Transaction::select(DB::raw("SUM(amount) as amount"))
              ...
              ->where('account_id', $account_id)->first()->amount;
    ...
    public function statementPdf()
    {
        ...
        $data['from'] = $from = isset($_GET['from']) ? $_GET['from'] : date("Y-m-d", strtotime ("-30 day", strtotime(date('d-m-Y') )));

        $data['to'] = $to = isset($_GET['to']) ? $_GET['to'] : date('Y-m-d');
        $data['account_no'] = $account_no = isset($_GET['account_no']) ? $_GET['account_no'] : NULL;
        $mode = isset($_GET['mode']) ?$_GET['mode']: null;
        $type = isset($_GET['type']) ?$_GET['type']: null;

        $data['transactionList'] = $this->transaction->getTransactionByAccountId($from, $to, $account_no, $mode, $type)
