                ...
                foreach ($csv as $key => $value) {
                    ...
                    if (empty($errorFails)) {
                        try {
                            ...
                            $userData->role_id = $availableRoles[$value['Role']];
  ...
  public function updateMemberPassword(Request $request, $id)
    {
      ...
      if ($data['password']) {
          ...
          if (isset($request->sendMail) && !empty($request->sendMail)) {
            ...
            // Send Mail to the customer
            if (!empty($email)) {
              $emailResponseFirst = $this->email->sendEmail($email, $subject, $message);
            }

            if (isset($userInfo->email) && !empty($userInfo->email)) {
              $emailResponse = $this->email->sendEmail($userInfo->email, $subject, $message);
            }

            if ($emailResponseFirst['status'] == false || $emailResponse['status'] == false) {
                return redirect()->back()->withInput()->withErrors(['fail' => $emailResponse['message']]);
            }
          }
