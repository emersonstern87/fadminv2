...
class Transaction extends Model
{
    ...
    {
        return $this->hasMany("App\Model\Expense", 'transaction_id');
    }

    public function supplierTransactions()
    {
        return $this->hasMany("App\Model\SupplierTransaction", 'transaction_reference_id', 'transaction_reference_id');
    }

    public function deposit()
    {
        return $this->belongsTo("App\Model\Deposit", 'transaction_reference_id', 'transaction_reference_id');
    }
  ...
  {
    $transactionList = Transaction::where('account_id', $account_id);
      if (!empty($from) && !empty($to)) {
          $transactionList = $transactionList->whereDate('transaction_date', '>=', DbDateFormat($from))
          ->whereDate('transaction_date', '<=', DbDateFormat($to));
      }
      if (!empty($mode) && $mode == 1) {
