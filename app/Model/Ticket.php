...
class Ticket extends Model
{
    ...
    {
        return $this->belongsTo('App\Model\TicketStatus','ticket_status_id');
    }
    ...
    {
        return $this->belongsTo('App\Model\User','assigned_member_id');
    }

    public function ticketReplies()
  {
      return $this->hasMany('App\Model\TicketReply','ticket_id');
  }
    ...
    public function getAllTicketDT($from, $to, $status, $project, $departmentId, $customerId = null, $flag = null, $assigneeId = null)
    {
        ...
        ->select('tickets.*', 'priorities.name as priority', 'priorities.id as priority_id', 'departments.name as department_name', 'ticket_statuses.name as status', 'ticket_statuses.color as color','projects.name as project_name','projects.id as project_table_id','customers.first_name as first_name','customers.last_name as last_name', 'users.full_name as assignee_name');
    ...
    public function getExceptClickedStatus($status, $customerId = null, $projectId = null) 
    {
    ...
    public function getFilteredStatus($options = []) 
    {
      ...
      if (isset($options['allstatus']) && !empty($options['allstatus']) ) {
        ...
        if (!empty($conditions['project_id'])) {
          $otherStatuses = $otherStatuses->where('tickets.project_id', $options['allproject']);
        } 
        ...
        if (isset($options['customerId']) && !empty($options['customerId'])) {
          $conditions['customer_id'] = $options['customerId'];
        }

        if (isset($options['allassignee']) && !empty($options['allassignee'])) {
          $conditions['assigned_member_id'] = $options['allassignee'];
        }
    ...
    public function getAllTicketDetailsById($id)
    {
      ...
      }, 'priority:id,name', 'department:id,name', 'ticketStatus:id,name,color',  'customer:id,email,name', 'project:id,name', 'user:id,email,full_name', 'assignedMember:id,email,full_name'])->where('id', $id)->first();
      return $res;
    }
