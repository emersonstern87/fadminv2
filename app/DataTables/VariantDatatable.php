...
class VariantDatatable extends DataTable
{
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('description', function ($item) {
                return '<a href="'.url('edit-item/item-info/'.$item->item_id).'">'.$item->name.'</a>';
            })

            ->addColumn('item_qty', function($item){
                if ($item->item_type != 'service') {
                    $qty = ($item->item_qty != null || $item->item_qty>0 ) ? formatCurrencyAmount($item->item_qty) : formatCurrencyAmount(0);
                    $qtyOnHand= $item->qty_on_hand ? formatCurrencyAmount($item->qty_on_hand) : formatCurrencyAmount(0);
                    $totalVariant=$item->total_variant ? formatCurrencyAmount($item->total_variant) : formatCurrencyAmount(0);
                    return $qty;
                } else {
                    return '-';
                }
            ...
            ->addColumn('retail_prices', function($item){
                $retail_sale_price = ($item->retail_sale_price != null || $item->retail_sale_price>0 ) ? formatCurrencyAmount($item->retail_sale_price) : formatCurrencyAmount(0);
            ...
            ->rawcolumns(['action','img','category','description','item_qty','purchase_price','retail_prices','wholesale_prices','inactive'])
    ...
    public function query()
    {
        ...
        $idArray = DB::table('items')->where('items.id', $itemID)->orWhere('items.parent_id', $itemID)->get(['id']);
        $idArray = array_column(json_decode(json_encode($idArray), true), 'id');
    ...
    public function html()
    {
        ...
        ->addColumn(['data' => 'item_id', 'name' => 'items.id', 'visible' => false])
        ...
        ->addColumn(['data' => 'retail_prices', 'name' => 'spr.price', 'title' => __('Retail').' '.($this->currency->symbol)])

        ->addColumn(['data' => 'inactive', 'title' => __('Status'), 'orderable' => false])
