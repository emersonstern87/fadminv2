    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('full_name', function ($users) {
                $full_name = '';
                $user = '';
                if (mb_strlen($users->full_name) > 10) {
                    $user_name = mb_substr($users->full_name, 0, 10). "..";
                } else {
                    $user_name = $users->full_name;
                }
                $full_name = $users->full_name;
                if (Helpers::has_permission(Auth::user()->id, 'edit_team_member')) {
                    $user .= mb_strlen($full_name) > 10 ? '<a href="'. url('user/team-member-profile/'. $users->id) .'"><span data-toggle="tooltip" data-placement="right" data-original-title="'. $full_name .'">'. $user_name .'</span></a><br>' : '<a href="'. url('user/team-member-profile/'. $users->id) .'">'. $user_name .'</a><br>';
                } else {
                    $user .= mb_strlen($full_name) > 10 ? '<span data-toggle="tooltip" data-placement="right" data-original-title="'. $full_name .'">'. $user_name .'</span>' : $user_name; 
                }
                return  $user;
            })
            ...
            ->addColumn('email', function ($users) {
                ...
                $user .= mb_strlen($full_mail) > 20 ? '<span data-toggle="tooltip" data-placement="right" data-original-title="'. $full_mail .'">'. $user_mail .'</span>' : $user_mail; 
                
                return  $user;
            })

            ->addColumn('created_at', function ($users) {
                $startDate = timeZoneformatDate($users->created_at);
                $startTime = timeZonegetTime($users->created_at);
