...
class LeadListDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('assigned', function ($leads) {
                if (!($leads->user)) {
                    ...
                    if (mb_strlen($leads->user->full_name) > 10) {
                        $realName = mb_substr($leads->user->full_name, 0, 10)."..";
                    } else {
                        $realName = $leads->user->full_name;
                    }
            ...
            ->addColumn('status', function ($leads) {
                if (!empty($leads->leadStatus)) {
                    $statusHolder = $leads->leadStatus->name;
                    $colorHolder = $leads->leadStatus->color;
            ...
            ->addColumn('tag', function ($leads) {
                    ...
                    foreach ($tagsInResult as $tagIn) {
                        $str = '<span class="label label-tag" ><span class="tag-in-result">'. $tagIn->tag->name .'</span></span>';
                        if ($str2 == "") {
                            $str2 .= $str; 
                        } else {
    ...
    public function html()
    {
        return $this->builder()
            ...
            ->addColumn(['data' => 'name', 'name' => 'last_name', 'visible' =>false])
