...
class AddForeignKeysToReceivedOrderDetailsTable extends Migration {
	...
	public function up()
	{
		Schema::table('received_order_details', function(Blueprint $table)
		{
			...
			$table->foreign('purchase_order_detail_id')->references('id')->on('purchase_order_details')->onUpdate('CASCADE')->onDelete('NO ACTION');
			$table->foreign('purchase_order_id')->references('id')->on('purchase_orders')->onUpdate('CASCADE')->onDelete('NO ACTION');
			$table->foreign('received_order_id')->references('id')->on('received_orders')->onUpdate('CASCADE')->onDelete('CASCADE');
		});
	}
