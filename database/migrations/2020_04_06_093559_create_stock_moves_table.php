...
class CreateStockMovesTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id');
			$table->integer('item_id')->unsigned()->index('stock_moves_item_id_foreign')->comment('item_id refers items table id');
			...
			$table->integer('user_id')->unsigned()->nullable()->index('stock_moves_user_id_foreign')->comment('user_id refers users table id');
