...
class AddForeignKeysToSalePricesTable extends Migration {
	...
	public function up()
	{
		Schema::table('sale_prices', function(Blueprint $table)
		{
			...
			$table->foreign('sale_type_id')->references('id')->on('sale_types')->onUpdate('CASCADE')->onDelete('NO ACTION');
			$table->foreign('currency_id', 'sales_prices_currency_id_foreign')->references('id')->on('currencies')->onUpdate('CASCADE')->onDelete('NO ACTION');
