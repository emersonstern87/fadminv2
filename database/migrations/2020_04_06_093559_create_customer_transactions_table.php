...
class CreateCustomerTransactionsTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id');
			$table->integer('user_id')->unsigned()->nullable()->index('customer_transactions_user_id_foreign');
			$table->integer('account_id')->unsigned()->nullable()->index('customer_transactions_account_id_foreign');
			$table->integer('payment_method_id')->unsigned()->nullable()->index('customer_transactions_payment_method_id_foreign_idx');
			$table->integer('customer_id')->unsigned()->index('customer_transactions_customer_id_foreign');
			$table->integer('sale_order_id')->unsigned()->index('customer_transactions_invoice_id_foreign_idx');
