...
class CreateSupplierTransactionsTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('id');
			$table->integer('user_id')->unsigned()->nullable()->index('supplier_transactions_user_id_foreign');
			...
			$table->integer('supplier_id')->unsigned()->index('supplier_transactions_supplier_id_foreign');
			$table->integer('purchase_order_id')->unsigned()->index('supplier_transactions_purchase_order_id_foreign_idx');
