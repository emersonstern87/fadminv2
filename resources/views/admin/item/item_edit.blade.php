...
<div class="col-sm-12" id="edit-item-container">
  <div class="card user-list">
    <div class="card-header">
      <h5><a href="{{ url('item') }}">{{ __('Items')  }}</a> >> <a href='{{ url("edit-item/variant/$primaryItem->id") }}'> {{$primaryItem->name}}</a> </h5>
                  ...
                  <tbody>
                    @if(!empty($locData))
                      ...
                      @foreach ($locData as $data)
                        <tr>
                          <td>{{$data->name}}</td>
                          <td>{{ formatCurrencyAmount( (new App\Model\StockMove)->getItemQtyByLocationName($data->id, $itemInfo->id) )}}</td>
                        </tr>
                        <?php
                          $sum += (new App\Model\StockMove)->getItemQtyByLocationName($data->id, $itemInfo->id);
                        ?>
                      @endforeach
                    @endif
                    <tr>
