      ...
      <div class="col-sm-12 m-t-10">
        <form class="form-horizontal" action="{{ url('ticket/list') }}" method="GET" accept-charset="UTF-8">
          ...
          <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 m-l-10">
            <div class="row mt-3">
              <div class="ticket-filter col-md-12 col-xl-3 col-lg-4 col-sm-12 col-xs-12 mb-2">
            ...
            @endphp
              <div class="col-md-2 ticket-summary text-white m-l-25 pr-md-0" id="ticket-name">
                <div class="text-center p-10 mt-1 status-border" style="color: {{ $color }} ">
                    ...
                    <a href="{{ url('ticket/list?from='.$from.'&to='.$to.'&project='.$project.'&department='.$department.'&status='.$data->id.'&btn=') }}" style="color: {{ $color }}"><span class="f-20" id="{{ str_replace(' ', '', $data->name) }}">{{ $data->total_status }}</span></a>
                </div>
              </div>
          @endforeach    
        </div>
