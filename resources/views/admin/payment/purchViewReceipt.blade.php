...
<div id="purchase-payment-view-details-container">
  ...
  <div id="emailReceipt" class="modal fade" role="dialog">
    @inject('purchaseOrder', 'App\Model\PurchaseOrder')
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input type="email" value="{{ old('email') ? old('email') : $paymentInfo->supplier->email }}" class="form-control" name="email" id="email">
          </div>
          <?php
              $subjectText = str_replace('{purchase_reference_no}', $paymentInfo->purchaseOrder->reference, $emailInfo->subject);
          ...
          <div class="form-group">
            @php
              $bodyInfo = str_replace('{supplier_name}', $paymentInfo->supplier->name, $emailInfo->body);
              ...
              $bodyInfo = str_replace('{purchase_reference_no}', $paymentInfo->purchaseOrder->reference, $bodyInfo);
              $bodyInfo = str_replace('{company_name}', $company_name, $bodyInfo);
              $bodyInfo = str_replace('{billing_state}', $paymentInfo->supplier->city, $bodyInfo);                      
              $bodyInfo = str_replace('{billing_street}', $paymentInfo->supplier->street, $bodyInfo);                      
              $bodyInfo = str_replace('{billing_city}', $paymentInfo->supplier->state, $bodyInfo);                      
              $bodyInfo = str_replace('{billing_zip_code}', $paymentInfo->supplier->zipcode, $bodyInfo);                       
              $bodyInfo = str_replace('{billing_country}', $paymentInfo->supplier->Country->name, $bodyInfo);
            @endphp
            <textarea id="compose-textarea" name="message" id='message' class="form-control editor h-200">{{$bodyInfo}}</textarea>
          </div>
          <div class="form-group">
            <div class="checkbox">
              <label><input type="checkbox" name="payment_pdf" checked><strong>{{'PM-'. sprintf("%04d", $paymentInfo->id)}}</strong></label>
            </div>
          </div>
        </div>
